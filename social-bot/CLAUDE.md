...
- **Food/drink days come from `data/food_days.json`, shared with the digest bot** (`active_food_day_hints`). Add a day once and BOTH bots pick it up. It's shared as *data*, not code — the two bots remain independent modules with no cross-imports. Hints prime the research toward that day's specials/collabs (find the specific venue actually doing something + the real post; never a generic "it's X day" mention).
