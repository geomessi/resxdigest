...
3. **`research_ugc()`** — one large Claude+web_search call that carries the *entire* editorial judgment in a single prompt: tier-prioritized discovery (celebrity sightings and viral restaurant moments first, FOMO second, lifestyle moments rare), source-quality rules (real Reels/TikTok/accounts over generic articles), pinned-lead resolution, six action-type classification, trending-audio discovery, and honest 1-5 self-scoring across 5 axes. Returns opportunities, audio, `pinned_rejected`, and `considered_and_rejected`.
...
6. **`apply_diversity()`** — caps the digest at one item per `subject` (venue/topic/creator/song), pinned items processed first so they win any conflict.
7. **`dedupe_audio()`** — collapses duplicate song+artist within the same run.
8. **`build_slack_blocks()`** — a flat, most-compelling-first list (no city or type grouping), each item tagged inline with its action type (`→ REPOST · NYC`), plus a "Trending Audio" section at the bottom. Adds a forced-rerun banner if `FORCE_POST=1` overrode an already-completed day.
9. **`post_to_slack()`** — skipped entirely under `DRY_RUN=1`, which instead prints the exact payload.
10. **Persist state** — `seen_ugc.json`, `last_social_post.json`, `social_pinned_leads.json` (cleared), `social_skipped_log.json` — all skipped under `DRY_RUN=1`.
...
- **Split pinned-lead resolution out of the single big `research_ugc` call**, mirroring the digest bot's dedicated `research_pinned_inputs`. Right now a pinned lead's fate depends on the same prompt that's simultaneously juggling six action types, trending audio, tiering, and scoring — a dedicated call would likely be more reliable, the same way splitting it out improved the digest bot's version.
