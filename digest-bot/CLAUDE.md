...
- **Food/drink days live in `data/food_days.json`, shared with `social_bot.py`** (`active_food_day_hints`). Add a day once and BOTH bots pick it up. `CALENDAR` in `bot.py` is now holidays/observances ONLY (the ones that can set a `special_header`) — do **not** add food days there, they'd be ignored. Shared as *data*, not code; the bots stay independent modules.
